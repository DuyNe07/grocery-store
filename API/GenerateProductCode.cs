...
namespace grocery_store.API
{
    ...
    {
        public async Task<string> GenerateSKU(string productName, string category, string supplier)
        {
            // Băm lần lượt name, category, supplier và ghép lại để tạo SKU
            string nameHash = await GenerateHash(productName);
            string categoryHash = await GenerateHash(category);
            string supplierHash = await GenerateHash(supplier);
            // Tạo SKU từ các chuỗi hash
            string sku = $"{nameHash}.{categoryHash}.{supplierHash}";
            return sku;
        }

        private async Task<string> GenerateHash(string input)
        {
